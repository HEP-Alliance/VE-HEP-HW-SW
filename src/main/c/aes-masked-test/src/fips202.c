 ...
 **************************************************/
static void
keccak_absorb(uint64_t* s, uint32_t r, const uint8_t* m, size_t mlen, uint8_t p)
{
  size_t i;
  uint8_t t[200];
...
void cshake128_simple(uint8_t* output, size_t outlen, uint16_t cstm, const uint8_t* in, size_t inlen)
{
  ...
  if (outlen % SHAKE128_RATE) {
    ...
    for (i = 0; i < outlen % SHAKE128_RATE; i++) output[i] = t[i];
  }
#ifdef PROFILE_HASHING
  uint64_t t1 = hal_get_time();
  hash_cycles += (t1 - t0);
...
void sha3_256_inc_finalize(uint8_t* output, sha3_256incctx* state)
{
  ...
  for (size_t i = 0; i < 32; i++) { output[i] = t[i]; }
#ifdef PROFILE_HASHING
  uint64_t t1 = hal_get_time();
  hash_cycles += (t1 - t0);
#endif
...
void sha3_384_inc_finalize(uint8_t* output, sha3_384incctx* state)
{
  ...
  for (size_t i = 0; i < 48; i++) { output[i] = t[i]; }
#ifdef PROFILE_HASHING
  uint64_t t1 = hal_get_time();
  hash_cycles += (t1 - t0);
#endif
...
void sha3_512_inc_finalize(uint8_t* output, sha3_512incctx* state)
{
  ...
  for (size_t i = 0; i < 64; i++) { output[i] = t[i]; }
#ifdef PROFILE_HASHING
  uint64_t t1 = hal_get_time();
  hash_cycles += (t1 - t0);
#endif
...
void cshake256_simple(uint8_t* output, size_t outlen, uint16_t cstm, const uint8_t* in, size_t inlen)
{
  ...
  if (outlen % SHAKE256_RATE) {
    ...
    for (i = 0; i < outlen % SHAKE256_RATE; i++) output[i] = t[i];
  }
#ifdef PROFILE_HASHING
  uint64_t t1 = hal_get_time();
  hash_cycles += (t1 - t0);
